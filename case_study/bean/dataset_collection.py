...
class Dataset:
    ...
    def get_degree2nodes_dict(self):
        return self.dataset_obs.get_degree2nodes_dict()
...
class DataSetObs:
    ...
    def get_degree2nodes_dict(self):
        degree2nodes_dict: dict[int, list[int]] = dict()

        for node, set_of_edges in self.relationship_obs.node_to_set_of_edges_dict.items():
            degree: int = len(set_of_edges)
            if degree not in degree2nodes_dict.keys():
                degree2nodes_dict[degree] = list()
            degree2nodes_dict[degree].append(node)

        # Sorting the dictionary by its keys (degree) in ascending order
        sorted_degree2nodes_dict = {degree: nodes for degree, nodes in sorted(degree2nodes_dict.items())}

        return sorted_degree2nodes_dict
