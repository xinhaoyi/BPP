...
from case_study.bean.data_version import DataVersion
    ...
    def print_res(self):
        ...
        for model_name, indicator_dict in self.res.items():
            ...
            for indicator_name, score in self.res[model_name].items():
                ...
                print("- - - -" + "- - - -" + indicator_name + ": " + '{:.2f}'.format(float(score)))



def case_study(toplevel_pathway_name: str, relationship: Relationship, data_version: DataVersion, model: str):
...
def case_study_on_dataset(dataset_name: str):
    ...
    # data_old = DataVersion("data")
    data_new = DataVersion("data_version_85")
    data_comparator = DataComparator(old_data=data_old, new_data=data_new)
    new_relationships = data_comparator.choose_dataset(dataset_name).get_relationships_newly_added_with_node_and_edge_in_old_data()
...
def case_study_on_primary_and_secondary_entities(toplevel_pathway_name:str, dataset_name: str, data_version:DataVersion, threshold_degree: int):
    degree2nodes_dict = node_service.get_degree2node_dict(toplevel_pathway_name, data_version)
    primary_nodes_indexes, secondary_nodes_indexes = PrimarySecondaryEntityEngine().get_primary_secondary_entities(degree2nodes_dict, threshold_degree)
    ...
    print("-----------------------------case study on primary entities of " + dataset_name + " -----------------------------")
    ...
    print("-----------------------------case study on secondary entities of " + dataset_name + " -----------------------------")
