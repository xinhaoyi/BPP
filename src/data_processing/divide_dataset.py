...
class ReactomeDataDivider:
    def __init__(self, pathway_name):
        ...
        (
            self.__input_link_prediction_reaction_ids,
            self.__output_link_prediction_reactions_ids,
            self.__regulation_link_prediction_reactions_ids,
        ) = self.__get_two_divided_reaction_ids()
    ...
    def __ultimate_initialisation(self):
        ...
        self.__list_of_pair_of_entity_and_component = (
            self.__read_all_pair_of_entity_and_component_of_one_pathway_from_file()
        )
    ...
    def __read_reactions_of_one_pathway_from_file(self) -> list[str]:
        reactions_ids = self.__file_processor.read_file_via_lines(
            "data/" + self.__pathway_name + "/", self.__edges_file_name
        )
        return reactions_ids

    def __read_entities_of_one_pathway_from_file(self) -> list[str]:
        entities_ids = self.__file_processor.read_file_via_lines(
            "data/" + self.__pathway_name + "/", self.__nodes_file_name
        )
        return entities_ids
    ...
    def __read_all_relationships_of_one_pathway_from_file(self) -> list[list[str]]:
        relationships_string_style = self.__file_processor.read_file_via_lines(
            "data/" + self.__pathway_name + "/", self.__relationship_file_name
        )
    ...
    def __read_all_components_of_one_pathway_from_file(self) -> list[str]:
        component_ids = self.__file_processor.read_file_via_lines(
            "data/" + self.__pathway_name + "/", self.__all_components_file_name
        )
        return component_ids

    # list of [entity_id, component_id]
    def __read_all_pair_of_entity_and_component_of_one_pathway_from_file(
        self,
    ) -> list[list[str]]:
        # 355,1190,1209
        list_of_entity_components_mappings_with_index_style = (
            self.__file_processor.read_file_via_lines(
        ...
        for i in range(len(list_of_entity_components_mappings_with_index_style)):
            ...
            for component_str in list_of_component_index_str_style:
                ...
                list_of_pair_of_entity_and_component.append(
                    line_list_of_entity_id_and_component_id
                )
    ...
    def __initialisation_inner_reaction_to_list_of_entities_and_entity_to_list_of_reactions_dict(
        self,
    ):
        ...
        for relationship in self.__relationships:
            ...
            if int(eval(direction)) < 0:
                ...
                if reaction_id in self.__reaction_to_list_of_input_entities_dict.keys():
                    entities_list = self.__reaction_to_list_of_input_entities_dict[
                        reaction_id
                    ]
                    entities_list.append(entity_id)
                else:
                ...
                if entity_id in self.__entity_to_list_of_input_reactions_dict.keys():
                    reactions_list = self.__entity_to_list_of_input_reactions_dict[
                        entity_id
                    ]
                    reactions_list.append(reaction_id)
                else:
            ...
            # direction = 1, output
                ...
                if (
                    reaction_id
                    in self.__reaction_to_list_of_output_entities_dict.keys()
                ):
                    entities_list = self.__reaction_to_list_of_output_entities_dict[
                        reaction_id
                    ]
                    entities_list.append(entity_id)
                else:
                ...
                if entity_id in self.__entity_to_list_of_output_reactions_dict.keys():
                    reactions_list = self.__entity_to_list_of_output_reactions_dict[
                        entity_id
                    ]
                    reactions_list.append(reaction_id)
                else:
            ...
            # direction = 0, regulation
                ...
                if (
                    reaction_id
                    in self.__reaction_to_list_of_regulation_entities_dict.keys()
                ):
                    entities_list = self.__reaction_to_list_of_regulation_entities_dict[
                        reaction_id
                    ]
                    entities_list.append(entity_id)
                else:
                ...
                if (
                    entity_id
                    in self.__entity_to_list_of_regulation_reactions_dict.keys()
                ):
                    reactions_list = self.__entity_to_list_of_regulation_reactions_dict[
                        entity_id
                    ]
                    reactions_list.append(reaction_id)
                else:
    ...
    def __initialisation_inner_entity_and_component_dict(self):
        ...
        for pair_of_entity_and_component in self.__list_of_pair_of_entity_and_component:
            entity_id = pair_of_entity_and_component[
                self.__entity_index_of_pair_of_entity_and_component
            ]
            component_id = pair_of_entity_and_component[
                self.__component_index_of_pair_of_entity_and_component
            ]
    ...
    def __get_inner_reaction_to_list_of_entities_and_entity_to_list_of_reactions_dict(
        self, relationships
    ):
        """initialise the inner dictionary of reaction to entities and entity to reactions based on different direction
        This method initialise the following inner dictionaries, and will be called by self.__initialisation_set_reactions_entities_and_components_dict(self)
        ...
        for relationship in relationships:
            ...
            # direction = 1, output
                ...
                if reaction_id in reaction_to_list_of_output_entities_dict.keys():
                    entities_list = reaction_to_list_of_output_entities_dict[
                        reaction_id
                    ]
                    entities_list.append(entity_id)
                else:
                    entities_list = list()
                    entities_list.append(entity_id)
                    reaction_to_list_of_output_entities_dict[
                        reaction_id
                    ] = entities_list
            ...
            # direction = 0, regulation
                ...
                if reaction_id in reaction_to_list_of_regulation_entities_dict.keys():
                    entities_list = reaction_to_list_of_regulation_entities_dict[
                        reaction_id
                    ]
                    entities_list.append(entity_id)
                else:
                    entities_list = list()
                    entities_list.append(entity_id)
                    reaction_to_list_of_regulation_entities_dict[
                        reaction_id
                    ] = entities_list

                # entity to list of regulation reactions dict
                if entity_id in entity_to_list_of_regulation_reactions_dict.keys():
                    reactions_list = entity_to_list_of_regulation_reactions_dict[
                        entity_id
                    ]
                    reactions_list.append(reaction_id)
                else:
                    reactions_list = list()
                    reactions_list.append(reaction_id)
                    entity_to_list_of_regulation_reactions_dict[
                        entity_id
                    ] = reactions_list

        return (
            reaction_to_list_of_entities_dict,
            entity_to_list_of_reactions_dict,
            reaction_to_list_of_input_entities_dict,
            entity_to_list_of_input_reactions_dict,
            reaction_to_list_of_output_entities_dict,
            entity_to_list_of_output_reactions_dict,
            reaction_to_list_of_regulation_entities_dict,
            entity_to_list_of_regulation_reactions_dict,
        )

    def __get_inner_entity_and_component_dict(
        self, list_of_pair_of_entity_and_component: list[list[str]]
    ):
        """
        self.__all_entity_to_list_of_components_dict: dict[str, list[str]] = {}
        self.__all_component_to_list_of_entities_dict: dict[str, list[str]] = {}
    ...
    def test_attributes(self):
        ...
        for (
            component,
            list_of_entities,
        ) in self.__component_to_list_of_entities_dict.items():
        ...
        print(
            "more than 8: ",
            str(num_9) + "  " + "{:.2%}".format(float(num_9 / totol_num)),
        )

    def __delete_pair_of_entity_and_component(
        self, pair_of_entity_and_component: list[str]
    ):
        # self.__list_of_pair_of_entity_and_component
    ...
    def __delete_relationship(self, relationship: list[str]):
        ...
        if int(eval(direction)) < 0:
            self.__reaction_to_list_of_input_entities_dict[reaction_id].remove(
                entity_id
            )
            self.__entity_to_list_of_input_reactions_dict[entity_id].remove(reaction_id)
        elif int(eval(direction)) > 0:
            self.__reaction_to_list_of_output_entities_dict[reaction_id].remove(
                entity_id
            )
            self.__entity_to_list_of_output_reactions_dict[entity_id].remove(
                reaction_id
            )
        else:
            self.__reaction_to_list_of_regulation_entities_dict[reaction_id].remove(
                entity_id
            )
            self.__entity_to_list_of_regulation_reactions_dict[entity_id].remove(
                reaction_id
            )
    ...
    def __input_link_prediction_delete_relationship(self, relationship: list[str]):
        ...
        self.__input_link_prediction_reaction_to_list_of_entities_dict[
            reaction_id
        ].remove(entity_id)
        self.__input_link_prediction_entity_to_list_of_reactions_dict[entity_id].remove(
            reaction_id
        )

        if int(eval(direction)) < 0:
            self.__input_link_prediction_reaction_to_list_of_input_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__input_link_prediction_entity_to_list_of_input_reactions_dict[
                entity_id
            ].remove(reaction_id)
        elif int(eval(direction)) > 0:
            self.__input_link_prediction_reaction_to_list_of_output_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__input_link_prediction_entity_to_list_of_output_reactions_dict[
                entity_id
            ].remove(reaction_id)
        else:
            self.__input_link_prediction_reaction_to_list_of_regulation_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__input_link_prediction_entity_to_list_of_regulation_reactions_dict[
                entity_id
            ].remove(reaction_id)
    ...
    def __output_link_prediction_delete_relationship(self, relationship: list[str]):
        ...
        self.__output_link_prediction_reaction_to_list_of_entities_dict[
            reaction_id
        ].remove(entity_id)
        self.__output_link_prediction_entity_to_list_of_reactions_dict[
            entity_id
        ].remove(reaction_id)

        if int(eval(direction)) < 0:
            self.__output_link_prediction_reaction_to_list_of_input_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__output_link_prediction_entity_to_list_of_input_reactions_dict[
                entity_id
            ].remove(reaction_id)
        elif int(eval(direction)) > 0:
            self.__output_link_prediction_reaction_to_list_of_output_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__output_link_prediction_entity_to_list_of_output_reactions_dict[
                entity_id
            ].remove(reaction_id)
        else:
            self.__output_link_prediction_reaction_to_list_of_regulation_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__output_link_prediction_entity_to_list_of_regulation_reactions_dict[
                entity_id
            ].remove(reaction_id)
    ...
    def __regulation_link_prediction_delete_relationship(self, relationship: list[str]):
        ...
        self.__regulation_link_prediction_reaction_to_list_of_entities_dict[
            reaction_id
        ].remove(entity_id)
        self.__regulation_link_prediction_entity_to_list_of_reactions_dict[
            entity_id
        ].remove(reaction_id)

        if int(eval(direction)) < 0:
            self.__regulation_link_prediction_reaction_to_list_of_input_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__regulation_link_prediction_entity_to_list_of_input_reactions_dict[
                entity_id
            ].remove(reaction_id)
        elif int(eval(direction)) > 0:
            self.__regulation_link_prediction_reaction_to_list_of_output_entities_dict[
                reaction_id
            ].remove(entity_id)
            self.__regulation_link_prediction_entity_to_list_of_output_reactions_dict[
                entity_id
            ].remove(reaction_id)
        else:
            self.__regulation_link_prediction_reaction_to_list_of_regulation_entities_dict[
                reaction_id
            ].remove(
                entity_id
            )
            self.__regulation_link_prediction_entity_to_list_of_regulation_reactions_dict[
                entity_id
            ].remove(
                reaction_id
            )
    ...
    def __get_list_of_relationships_based_on_list_of_reaction_ids(
        self, list_of_reaction_ids
    ):
        list_of_relationships: list[list[str]] = list()
        for reaction_id in list_of_reaction_ids:
            list_of_relationships_for_single_reaction_id = (
                self.__get_list_of_relationships_based_on_reaction_id(reaction_id)
            )
            list_of_relationships.extend(list_of_relationships_for_single_reaction_id)

        list_of_relationships = list(
            set(tuple(relationship) for relationship in list_of_relationships)
        )

        # every edge from tuple to list
        list_of_relationships = list(
            list(relationship) for relationship in list_of_relationships
        )

        return list_of_relationships

    def __get_list_of_entities_based_on_list_of_relationships(
        self, list_of_relationships: list[list[str]]
    ) -> list[str]:
        set_of_entities = set()
    ...
    def __get_list_of_components_based_on_list_of_entity_ids(self, list_of_entity_ids):
        ...
        for entity_id in list_of_entity_ids:
            list_of_components_for_single_entity = (
                self.__entity_to_list_of_components_dict[entity_id]
            )
            list_of_components.extend(list_of_components_for_single_entity)

        return list(set(list_of_components))

    def __get_list_of_pair_of_entity_and_component_based_on_entity_id(
        self, entity_id: str
    ):
        ...
        for pair_of_entity_and_component in self.__list_of_pair_of_entity_and_component:
            if (
                ...
                list_of_pair_of_entity_and_component_for_return.append(
                    copy.deepcopy(pair_of_entity_and_component)
                )

        return list_of_pair_of_entity_and_component_for_return

    def __get_list_of_pair_of_entity_and_component_based_on_list_of_entity_ids(
        self, list_of_entity_ids: list[str]
    ):
        list_of_pair_of_entity_and_component = list()
        for entity_id in list_of_entity_ids:
            list_of_pair_of_entity_and_component_for_single_entity = (
                self.__get_list_of_pair_of_entity_and_component_based_on_entity_id(
                    entity_id
                )
            )
            list_of_pair_of_entity_and_component.extend(
                list_of_pair_of_entity_and_component_for_single_entity
            )

        list_of_pair_of_entity_and_component = list(
            set(
                tuple(pair_of_entity_and_component)
                for pair_of_entity_and_component in list_of_pair_of_entity_and_component
            )
        )
        ...
        # every edge from tuple to list
        list_of_pair_of_entity_and_component = list(
            list(pair_of_entity_and_component)
            for pair_of_entity_and_component in list_of_pair_of_entity_and_component
        )
    ...
    def divide_data_for_attribute_prediction_task(self):
        train_data_bean = DataBeanForReactome(
            self.__pathway_name,
            self.__attribute_prediction_task,
            self.__train_type_divided_dataset,
            self,
        )
        validation_data_bean = DataBeanForReactome(
            self.__pathway_name,
            self.__attribute_prediction_task,
            self.__validation_type_divided_dataset,
            self,
        )
        test_data_bean = DataBeanForReactome(
            self.__pathway_name,
            self.__attribute_prediction_task,
        ...
        while validation_counter < validation_size or test_counter < test_size:
            ...
            list_of_components = self.__entity_to_list_of_components_dict.get(
                random_entity_id
            )
            ...
            if len(list_of_components) >= 2:
                ...
                list_of_entities = self.__component_to_list_of_entities_dict[
                    random_component_id
                ]
                if len(list_of_entities) >= 2:
                    ...
                    list_of_relationships = (
                        self.__get_list_of_relationships_based_on_entity_id(
                            random_entity_id
                        )
                    )
                    ...
                    if validation_counter >= validation_size:
                        ...
                        test_data_bean.add_pair_of_entity_and_component(
                            pair_of_entity_and_component
                        )
                        test_counter = test_counter + 1
                    elif test_counter >= test_size:
                        validation_data_bean.add_list_of_relationships(
                            list_of_relationships
                        )
                        validation_data_bean.add_pair_of_entity_and_component(
                            pair_of_entity_and_component
                        )
                        ...
                        if flag == 0:
                            test_data_bean.add_list_of_relationships(
                                list_of_relationships
                            )
                            test_data_bean.add_pair_of_entity_and_component(
                                pair_of_entity_and_component
                    ...
                    self.__delete_pair_of_entity_and_component(
                        pair_of_entity_and_component
                    )
        ...
        validation_data_bean.generate_and_print_components_mapping_mix_negative_to_file(
            10
        )
    ...
    def __get_three_divided_reaction_ids(self):
        ...
        num_of_regulation_link_prediction_reaction_ids = (
            total_num
            - num_of_input_link_prediction_reaction_ids
            - num_of_output_link_prediction_reaction_ids
        )

        output_link_prediction_reaction_ids_start_index = (
            input_link_prediction_reaction_ids_end_index
        ) = num_of_input_link_prediction_reaction_ids

        regulation_link_prediction_reaction_ids_start_index = (
            output_link_prediction_reaction_ids_end_index
        ) = (
            num_of_input_link_prediction_reaction_ids
            + num_of_output_link_prediction_reaction_ids
        )

        input_link_prediction_reaction_ids = reactions_ids[
            0:input_link_prediction_reaction_ids_end_index
        ]
        ...
        return (
            input_link_prediction_reaction_ids,
            output_link_prediction_reactions_ids,
            regulation_link_prediction_reactions_ids,
        )
    ...
    def __get_two_divided_reaction_ids(self):
        ...
        num_of_output_link_prediction_reaction_ids = (
            total_num - num_of_input_link_prediction_reaction_ids
        )
    ...
    def __input_link_prediction_initialise_reactions_and_entities_components_and_relationships_and_list_of_pair_of_entity_and_component(
        ...
        self.__input_link_prediction_relationships = (
            self.__get_list_of_relationships_based_on_list_of_reaction_ids(reaction_ids)
        )
        self.__input_link_prediction_entity_ids = (
            self.__get_list_of_entities_based_on_list_of_relationships(
                self.__input_link_prediction_relationships
            )
        )

        self.__input_link_prediction_list_of_pair_of_entity_and_component = (
            self.__get_list_of_pair_of_entity_and_component_based_on_list_of_entity_ids(
                self.__input_link_prediction_entity_ids
            )
        )

        self.__input_link_prediction_component_ids = (
            self.__get_list_of_components_based_on_list_of_entity_ids(
                self.__input_link_prediction_entity_ids
            )
        )
        ...
        (
            self.__input_link_prediction_reaction_to_list_of_entities_dict,
            self.__input_link_prediction_entity_to_list_of_reactions_dict,
            self.__input_link_prediction_reaction_to_list_of_input_entities_dict,
            self.__input_link_prediction_entity_to_list_of_input_reactions_dict,
            self.__input_link_prediction_reaction_to_list_of_output_entities_dict,
            self.__input_link_prediction_entity_to_list_of_output_reactions_dict,
            self.__input_link_prediction_reaction_to_list_of_regulation_entities_dict,
            self.__input_link_prediction_entity_to_list_of_regulation_reactions_dict,
        ) = self.__get_inner_reaction_to_list_of_entities_and_entity_to_list_of_reactions_dict(
            self.__input_link_prediction_relationships
        )

        (
            self.__input_link_prediction_entity_to_list_of_components_dict,
            self.__input_link_prediction_component_to_list_of_entities_dict,
        ) = self.__get_inner_entity_and_component_dict(
            self.__input_link_prediction_list_of_pair_of_entity_and_component
        )
    ...
    def __output_link_prediction_initialise_reactions_and_entities_components_and_relationships_and_list_of_pair_of_entity_and_component(
        ...
        self.__output_link_prediction_component_ids = (
            self.__get_list_of_components_based_on_list_of_entity_ids(
                self.__output_link_prediction_entity_ids
            )
        )
        ...
        (
            self.__output_link_prediction_entity_to_list_of_components_dict,
            self.__output_link_prediction_component_to_list_of_entities_dict,
        ) = self.__get_inner_entity_and_component_dict(
            self.__output_link_prediction_list_of_pair_of_entity_and_component
        )
    ...
    def __regulation_link_prediction_initialise_reactions_and_entities_components_and_relationships_and_list_of_pair_of_entity_and_component(
        ...
        self.__regulation_link_prediction_component_ids = (
            self.__get_list_of_components_based_on_list_of_entity_ids(
                self.__regulation_link_prediction_entity_ids
            )
        )
        ...
        (
            self.__output_link_prediction_entity_to_list_of_components_dict,
            self.__regulation_link_prediction_component_to_list_of_entities_dict,
        ) = self.__get_inner_entity_and_component_dict(
            self.__regulation_link_prediction_list_of_pair_of_entity_and_component
        )
    ...
    def divide_data_for_input_link_prediction_task(self):
        train_data_bean = DataBeanForReactome(
            self.__pathway_name,
            self.__input_link_prediction_task,
            self.__train_type_divided_dataset,
            self,
        )
        validation_data_bean = DataBeanForReactome(
        ...
        for entity_id in self.__input_link_prediction_entity_ids:
            list_of_reactions = (
                self.__input_link_prediction_entity_to_list_of_input_reactions_dict.get(
                    entity_id
                )
            )
        ...
        while validation_counter < validation_size or test_counter < test_size:
            ...
            random_reaction_id = self.__input_link_prediction_reaction_ids[
                random_reaction_index
            ]
            ...
            if (
                len(list_of_input_entities) >= 2
                and random_reaction_id not in reaction_id_memory
            ):
                ...
                list_of_reactions = (
                    self.__input_link_prediction_entity_to_list_of_reactions_dict.get(
                        random_entity_id
                    )
                )
                if list_of_reactions is None:
                ...
                if len(list_of_reactions) >= 2 and (
                    random_entity_id not in entity_id_memory_dict
                    or entity_id_memory_dict[random_entity_id] < 5
                ):
                    if random_entity_id not in entity_id_memory_dict.keys():
                        entity_id_memory_dict[random_entity_id] = 1
                    else:
                        entity_id_memory_dict[random_entity_id] = (
                            entity_id_memory_dict.get(random_entity_id) + 1
                        )
        ...
        train_data_bean.add_list_of_relationships(
            self.__input_link_prediction_relationships
        )
        train_data_bean.add_list_of_pair_of_entity_and_component(
            self.__input_link_prediction_list_of_pair_of_entity_and_component
    ...
    def divide_data_for_output_link_prediction_task(self):
        train_data_bean = DataBeanForReactome(
            self.__pathway_name,
            self.__output_link_prediction_task,
            self.__train_type_divided_dataset,
            self,
        )
        validation_data_bean = DataBeanForReactome(
        ...
        for entity_id in self.__output_link_prediction_entity_ids:
            list_of_reactions = self.__output_link_prediction_entity_to_list_of_output_reactions_dict.get(
                entity_id
            )
            if list_of_reactions is None:
                list_of_reactions = list()
        ...
        while validation_counter < validation_size or test_counter < test_size:
            random_reaction_index = random.randint(0, end_index_of_reactions)
            random_reaction_id = self.__output_link_prediction_reactions_ids[
                random_reaction_index
            ]
            ...
            if (
                len(list_of_output_entities) >= 2
                and random_reaction_id not in reaction_id_memory
            ):
                # reaction_id_memory.add(random_reaction_id)
                random_entity_index = random.randint(
                    0, len(list_of_output_entities) - 1
                )
                random_entity_id = list_of_output_entities[random_entity_index]
                list_of_reactions = (
                    self.__output_link_prediction_entity_to_list_of_reactions_dict.get(
                        random_entity_id
                    )
                )
                if list_of_reactions is None:
    ...
    def divide_data_for_regulation_link_prediction_task(self):
        train_data_bean = DataBeanForReactome(
            self.__pathway_name,
            self.__regulation_link_prediction_task,
            self.__train_type_divided_dataset,
            self,
        )
        validation_data_bean = DataBeanForReactome(
        ...
        while validation_counter < validation_size or test_counter < test_size:
            random_reaction_index = random.randint(0, end_index_of_reactions)
            random_reaction_id = self.__regulation_link_prediction_reactions_ids[
                random_reaction_index
            ]
            ...
            if len(list_of_regulation_entities) >= 2:
                random_entity_index = random.randint(
                    0, len(list_of_regulation_entities) - 1
                )
...
class DataBeanForReactome:
    def __init__(
        self,
        pathway_name: str,
        task_of_sub_data_set: str,
        type_of_sub_data_set,
    ...
    def add_list_of_pair_of_entity_and_component(
        self, list_of_pair_of_entity_and_component: list[list[str]]
    ):
        for pair_of_entity_and_component in list_of_pair_of_entity_and_component:
    ...
    def __remove_duplicate_relationships(self):
        ...
        self.__relationships = list(
            set(tuple(relationship) for relationship in self.__relationships)
        )

        # every edge from tuple to list
        self.__relationships = list(
            list(relationship) for relationship in self.__relationships
        )
    ...
    def print_sub_data_bean_to_files(self):
        ...
        raw_reaction_id_to_reaction_index_dict = {
            reaction_id: index
            for index, reaction_id in enumerate(self.raw_data.get_raw_reaction_ids())
        }

        raw_entity_id_to_entity_index_dict = {
            entity_id: index
            for index, entity_id in enumerate(self.raw_data.get_raw_entities_ids())
        }

        raw_component_id_to_component_index_dict = {
            component_id: index
            for index, component_id in enumerate(self.raw_data.get_raw_component_ids())
        }
        ...
        for component_ids in self.__entities_component_ids_mapping_list:
            ...
            for component_id in component_ids:
                ...
                line_component_index_list = (
                    line_component_index_list + component_index + ","
                )
            # remove the comma in the end
            line_component_index_list = line_component_index_list[:-1]
            entities_component_indexes_mapping_list_for_print.append(
                line_component_index_list
            )
        ...
        relationships_index_style_for_print.sort(
            key=lambda l: int(re.findall("\d+", l)[1])
        )
        ...
        index_and_component_id_for_print.sort(
            key=lambda l: int(re.findall("\d+", l)[0])
        )
        ...
        pre_path = (
            "data/" + self.__pathway_name + "/" + self.__task_of_sub_data_set + "/"
        )
        self.__file_processor.createFile(
            pre_path + self.__type_of_sub_data_set, self.__all_components_file_name
        )
        self.__file_processor.createFile(
            pre_path + self.__type_of_sub_data_set,
            self.__entities_components_mapping_file_name,
        )
        self.__file_processor.createFile(
            pre_path + self.__type_of_sub_data_set, self.__edges_file_name
        )
        self.__file_processor.createFile(
            pre_path + self.__type_of_sub_data_set, self.__nodes_file_name
        )
        self.__file_processor.createFile(
            pre_path + self.__type_of_sub_data_set, self.__relationship_file_name
        )

        self.__file_processor.writeMessageToFile(
            pre_path + self.__type_of_sub_data_set,
    ...
    def generate_and_print_components_mapping_mix_negative_to_file(
        self, num_of_negative_elements: int
    ):

        entity_to_list_of_components_dict = copy.deepcopy(
            self.raw_data.get_entity_to_list_of_components_dict()
        )
        raw_component_ids = copy.deepcopy(self.raw_data.get_raw_component_ids())
        raw_component_id_to_component_index_dict = {
            component_id: index for index, component_id in enumerate(raw_component_ids)
        }
        ...
        for index, entity_id in enumerate(self.__entities_ids):
            ...
            negative_components_indexes: list[str] = [
                raw_component_id_to_component_index_dict[negative_components_id]
    ...
    def generate_and_print_relationships_mix_negative_to_file(
        self, num_of_negative_elements: int
    ):
        ...
        raw_entities_ids: list[str] = copy.deepcopy(
            self.raw_data.get_raw_entities_ids()
        )

        raw_reactions_ids: list[str] = copy.deepcopy(
            self.raw_data.get_raw_reaction_ids()
        )

        raw_entity_id_to_entity_index_dict = {
            entity_id: index for index, entity_id in enumerate(raw_entities_ids)
        }

        raw_reaction_id_to_reaction_index_dict = {
            reaction_id: index for index, reaction_id in enumerate(raw_reactions_ids)
        }
        ...
        for relationship_index_style in relationships_index_style_for_print:
            ...
            for negative_entity_id in negative_entities_list:
                negative_entity_index = raw_entity_id_to_entity_index_dict[
                    negative_entity_id
                ]

                negative_relationship_direction: int = (
                    1 if random.random() < 0.5 else -1
                )

                negative_relationship: str = (
                    str(negative_entity_index)
                    + ","
                    + str(reaction_index)
                    + ","
                    + str(negative_relationship_direction)
                )
            ...
            for (
                negative_relationship_index_style
            ) in negative_relationship_list_index_style:
                relationship_index_style_mix_negative = (
                    relationship_index_style_mix_negative
                    + "||"
                    + negative_relationship_index_style
                )

            relationship_index_style_list_mix_negative.append(
                relationship_index_style_mix_negative
            )
    ...
    def information(self):
        ...
        for list_of_entities in self.__reaction_to_list_of_entities_dict.values():
            ...
            if length > 4:
                reaction_with_relationship_information[5] = (
                    reaction_with_relationship_information[5] + 1
                )
            else:
                reaction_with_relationship_information[length] = (
                    reaction_with_relationship_information[length] + 1
                )

        print(
            "reaction with one relationship: "
            + str(reaction_with_relationship_information[1])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with two relationships: "
            + str(reaction_with_relationship_information[2])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with three relationships: "
            + str(reaction_with_relationship_information[3])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with four relationships: "
            + str(reaction_with_relationship_information[4])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with more than four relationships: "
            + str(reaction_with_relationship_information[5])
            + " "
            + "{:.2%}".format(
        ...
        for list_of_entities in self.__reaction_to_list_of_input_entities_dict.values():
            length = len(list_of_entities)
            if length > 4:
                reaction_with_input_relationship_information[5] = (
                    reaction_with_input_relationship_information[5] + 1
                )
            else:
                reaction_with_input_relationship_information[length] = (
                    reaction_with_input_relationship_information[length] + 1
                )

        print(
            "reaction with one input relationship: "
            + str(reaction_with_input_relationship_information[1])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with two input relationships: "
            + str(reaction_with_input_relationship_information[2])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with three input relationships: "
            + str(reaction_with_input_relationship_information[3])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with four input relationships: "
            + str(reaction_with_input_relationship_information[4])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "reaction with more than four input relationships: "
            + str(reaction_with_input_relationship_information[5])
            + " "
            + "{:.2%}".format(
        ...
        for (
            list_of_entities
        ) in self.__reaction_to_list_of_output_entities_dict.values():
            length = len(list_of_entities)
            if length > 4:
                reaction_with_output_relationship_information[5] = (
                    reaction_with_output_relationship_information[5] + 1
                )
            else:
                reaction_with_output_relationship_information[length] = (
                    reaction_with_output_relationship_information[length] + 1
                )

        # "  " + "{:.2%}".format(float(num_9 / totol_num)))
        print(
            "reaction with one output relationship: "
            + str(reaction_with_output_relationship_information[1])
            + " "
            + "{:.2%}".format(
                reaction_with_output_relationship_information[1]
                / total_num_of_reactions
            )
        )
        print(
            "reaction with two output relationships: "
            + str(reaction_with_output_relationship_information[2])
            + " "
            + "{:.2%}".format(
                reaction_with_output_relationship_information[2]
                / total_num_of_reactions
            )
        )
        print(
            "reaction with three output relationships: "
            + str(reaction_with_output_relationship_information[3])
            + " "
            + "{:.2%}".format(
                reaction_with_output_relationship_information[3]
                / total_num_of_reactions
            )
        )
        print(
            "reaction with four output relationships: "
            + str(reaction_with_output_relationship_information[4])
            + " "
            + "{:.2%}".format(
                reaction_with_output_relationship_information[4]
                / total_num_of_reactions
            )
        )
        print(
            "reaction with more output four output relationships: "
            + str(reaction_with_output_relationship_information[5])
            + " "
            + "{:.2%}".format(
                reaction_with_output_relationship_information[5]
                / total_num_of_reactions
        ...
        for list_of_reactions in self.__entity_to_list_of_reactions_dict.values():
            ...
            if length > 4:
                entity_with_relationships_information[5] = (
                    entity_with_relationships_information[5] + 1
                )
            else:
                entity_with_relationships_information[length] = (
                    entity_with_relationships_information[length] + 1
                )

        print(
            "entity with one relationship: "
            + str(entity_with_relationships_information[1])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "entity with two relationships: "
            + str(entity_with_relationships_information[2])
            + " "
            + "{:.2%}".format(
        ...
        print(
            ...
            + str(entity_with_relationships_information[3])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "entity with four relationships: "
            + str(entity_with_relationships_information[4])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "entity with more four relationships: "
            + str(entity_with_relationships_information[5])
            + " "
            + "{:.2%}".format(
        ...
        for list_of_components in self.__entity_to_list_of_components_dict.values():
            ...
            if length > 4:
                entity_with_components_information[5] = (
                    entity_with_components_information[5] + 1
                )
            else:
                entity_with_components_information[length] = (
                    entity_with_components_information[length] + 1
                )
        print(
            "entity with one component: "
            + str(entity_with_components_information[1])
            + " "
            + "{:.2%}".format(
        ...
        print(
            ...
            + str(entity_with_components_information[2])
            + " "
            + "{:.2%}".format(
        ...
        print(
            ...
            + str(entity_with_components_information[3])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "entity with four components: "
            + str(entity_with_components_information[4])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "entity with more four components: "
            + str(entity_with_components_information[5])
            + " "
            + "{:.2%}".format(
        ...
        for list_of_entities in self.__component_to_list_of_entities_dict.values():
            length = len(list_of_entities)
            if length > 4:
                compoent_with_entity_information[5] = (
                    compoent_with_entity_information[5] + 1
                )
            else:
                compoent_with_entity_information[length] = (
                    compoent_with_entity_information[length] + 1
                )

        print(
            "component with one entity: "
            + str(compoent_with_entity_information[1])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "component with two entities: "
            + str(compoent_with_entity_information[2])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "component with three entities: "
            + str(compoent_with_entity_information[3])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "component with four entities: "
            + str(compoent_with_entity_information[4])
            + " "
            + "{:.2%}".format(
        ...
        print(
            "component with more than four entities: "
            + str(compoent_with_entity_information[5])
            + " "
            + "{:.2%}".format(
...
class DataBean:
    def __init__(
        ...
        is_divided_dataset: bool = False,
        is_combination_dataset: bool = True,
        divided_dataset_task: str = None,
    ...
    def __check_data_bean_status(self):
        ...
        divided_dataset_task_allowance_list = [
        ...
        if (
            None is not self.__divided_dataset_task
            and self.__divided_dataset_task not in divided_dataset_task_allowance_list
        ):
            raise Exception(
        ...
        if (
            None is not self.__divided_dataset_type
            and self.__divided_dataset_type not in divided_dataset_type_allowance_list
        ):
            raise Exception(
    ...
    def __init_path(self):
        ...
        if (
            None is not self.__divided_dataset_task
            and None is not self.__divided_dataset_type
        ):
            self.__path = (
                self.__path
                + self.__divided_dataset_task
                + "/"
                + self.__divided_dataset_type
                + "/"
            )
    ...
    def __generate_inner_reactions_from_file(self) -> None:
        reactions_ids_file_name = self.__file_name_properties.get(
            "reactions_ids_file_name"
        )
        self.__reactions = self.__file_processor.read_file_via_lines(
            self.__path, reactions_ids_file_name
        )
    ...
    def __generate_inner_entities_from_file(self) -> None:
        entities_ids_file_name = self.__file_name_properties.get(
            "entities_ids_file_name"
        )
        self.__entities = self.__file_processor.read_file_via_lines(
            self.__path, entities_ids_file_name
        )

    #
    def __generate_inner_components_from_file(self) -> None:
        components_ids_file_name = self.__file_name_properties.get(
            "components_ids_file_name"
        )
        self.__components = self.__file_processor.read_file_via_lines(
            self.__path, components_ids_file_name
        )

    def __generate_inner_relationships_from_file(self) -> None:
        relationships_ids_file_name = self.__file_name_properties.get(
            "relationships_ids_file_name"
        )

        relationships_string_style = self.__file_processor.read_file_via_lines(
            self.__path, relationships_ids_file_name
        )
    ...
    def __generate_inner_pair_of_entity_and_component_from_file(self) -> None:
        ...
        for i in range(len(list_of_entity_components_mappings_with_index_style)):
            ...
            for component_str in list_of_component_index_str_style:
                ...
                line_list_of_entity_id_and_component_id.append(entity_id)
                line_list_of_entity_id_and_component_id.append(component_id)
                self.__list_of_pair_of_entity_and_component.append(
                    line_list_of_entity_id_and_component_id
                )
    ...
    def information(self):
        ...
        print("num of relationships: " + str(len(self.__relationships)) + "\n")
        print(
            "num of input relationships: " + str(len(self.__input_relationships)) + "\n"
        )
        print(
            "num of output relationships: "
            + str(len(self.__output_relationships))
            + "\n"
        )
        print(
            "num of regulation relationships: "
            + str(len(self.__regulation_relationships))
            + "\n"
        )

        temp_num_dict: dict[int, str] = {
            0: "zero",
            1: "one",
            2: "two",
            3: "three",
        ...
        for list_of_entities in self.__reaction_to_list_of_entities_dict.values():
            length = len(list_of_entities)
            if length > 8:
                reaction_with_relationship_information[9] = (
                    reaction_with_relationship_information[9] + 1
                )
            else:
                reaction_with_relationship_information[length] = (
                    reaction_with_relationship_information[length] + 1
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " entities: "
                + str(reaction_with_relationship_information[i])
                + " ("
                + "{:.2%}".format(
        ...
        for list_of_entities in self.__reaction_to_list_of_input_entities_dict.values():
            length = len(list_of_entities)
            if length > 8:
                reaction_with_input_relationship_information[9] = (
                    reaction_with_input_relationship_information[9] + 1
                )
            else:
                reaction_with_input_relationship_information[length] = (
                    reaction_with_input_relationship_information[length] + 1
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " input entities: "
                + str(reaction_with_input_relationship_information[i])
                + " ("
                + "{:.2%}".format(
                    reaction_with_input_relationship_information[i]
                    / total_num_of_reactions
                )
                + ")"
            )
        ...
        for (
            ...
            length = len(list_of_entities)
            if length > 8:
                reaction_with_output_relationship_information[9] = (
                    reaction_with_output_relationship_information[9] + 1
                )
            else:
                reaction_with_output_relationship_information[length] = (
                    reaction_with_output_relationship_information[length] + 1
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " output entities: "
                + str(reaction_with_output_relationship_information[i])
                + " ("
                + "{:.2%}".format(
                    reaction_with_output_relationship_information[i]
                    / total_num_of_reactions
                )
                + ")"
            )
        ...
        for (
            list_of_entities
        ) in self.__reaction_to_list_of_regulation_entities_dict.values():
            length = len(list_of_entities)
            if length > 8:
                ...
                reaction_with_regulation_relationship_information[length] = (
                    reaction_with_regulation_relationship_information[length] + 1
                )
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " output entities: "
                + str(reaction_with_regulation_relationship_information[i])
                + " ("
                + "{:.2%}".format(
                    reaction_with_regulation_relationship_information[i]
                    / total_num_of_reactions
                )
                + ")"
            )
        ...
        for list_of_reactions in self.__entity_to_list_of_reactions_dict.values():
            length = len(list_of_reactions)
            if length > 8:
                entity_with_relationships_information[9] = (
                    entity_with_relationships_information[9] + 1
                )
            else:
                entity_with_relationships_information[length] = (
                    entity_with_relationships_information[length] + 1
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " reactions: "
                + str(entity_with_relationships_information[i])
                + " ("
                + "{:.2%}".format(
        ...
        for list_of_reactions in self.__entity_to_list_of_input_reactions_dict.values():
            length = len(list_of_reactions)
            if length > 8:
                entity_with_input_relationships_information[9] = (
                    entity_with_input_relationships_information[9] + 1
                )
            else:
                entity_with_input_relationships_information[length] = (
                    entity_with_input_relationships_information[length] + 1
                )
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " input reactions: "
                + str(entity_with_input_relationships_information[i])
                + " ("
                + "{:.2%}".format(
                    entity_with_input_relationships_information[i]
                    / total_num_of_entities
                )
                + ")"
            )
        print("\n")
        ...
        for (
            list_of_reactions
        ) in self.__entity_to_list_of_output_reactions_dict.values():
            length = len(list_of_reactions)
            if length > 8:
                entity_with_output_relationships_information[9] = (
                    entity_with_output_relationships_information[9] + 1
                )
            else:
                entity_with_output_relationships_information[length] = (
                    entity_with_output_relationships_information[length] + 1
                )
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " output reactions: "
                + str(entity_with_output_relationships_information[i])
                + " ("
                + "{:.2%}".format(
                    entity_with_output_relationships_information[i]
                    / total_num_of_entities
                )
                + ")"
            )
        ...
        for (
            list_of_reactions
        ) in self.__entity_to_list_of_regulation_reactions_dict.values():
            length = len(list_of_reactions)
            if length > 8:
                entity_with_regulation_relationships_information[9] = (
                    entity_with_regulation_relationships_information[9] + 1
                )
            else:
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " output reactions: "
                + str(entity_with_regulation_relationships_information[i])
                + " ("
                + "{:.2%}".format(
                    entity_with_regulation_relationships_information[i]
                    / total_num_of_entities
                )
                + ")"
            )
        ...
        for list_of_components in self.__entity_to_list_of_components_dict.values():
            length = len(list_of_components)
            if length > 8:
                entity_with_components_information[9] = (
                    entity_with_components_information[9] + 1
                )
            else:
                entity_with_components_information[length] = (
                    entity_with_components_information[length] + 1
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " components: "
                + str(entity_with_components_information[i])
                + " ("
                + "{:.2%}".format(
        ...
        for list_of_entities in self.__component_to_list_of_entities_dict.values():
            length = len(list_of_entities)
            if length > 8:
                component_with_entities_information[9] = (
                    component_with_entities_information[9] + 1
                )
            else:
                component_with_entities_information[length] = (
                    component_with_entities_information[length] + 1
                )
        ...
        for i in range(0, 10):
            print(
                ...
                + temp_num_dict[i]
                + " entities: "
                + str(component_with_entities_information[i])
                + " ("
                + "{:.2%}".format(
    ...
    def test(self):
        print(
            "self.component_to_list_of_entities_dict: "
            + str(len(self.__component_to_list_of_entities_dict))
        )
